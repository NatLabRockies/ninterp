    ...
    S: for<'a> Strategy3D<ViewRepr<&'a D::Elem>> + Clone + Debug,
{
    ...
    fn validate(&self, data: &InterpData3DBase<D>) -> Result<(), ValidateError> {
        self.axes.validate_len(3, "GridTransform", "axes")?;
        let mut transformed_grid = Vec::with_capacity(3);
        for (dim, grid) in data.grid.iter().enumerate() {
    ...
    fn init(&mut self, data: &InterpData3DBase<D>) -> Result<(), ValidateError> {
        let mut grid_cache = Vec::with_capacity(3);
        for (dim, grid) in data.grid.iter().enumerate() {
            let transform = self.axes[dim];
            let mut transformed = grid.mapv(|x| transform.forward(x));
            if !transform.is_increasing() {
        ...
        D::Elem: Num + Euclid + Copy,
    {
        ...
        let wrapped: [D::Elem; 3] = core::array::from_fn(|i| {
